...
class OriginalArticle extends Component{
  ...
  constructor(props){
    ...
    this.state = {
      ...
      url: ip + '/common/user_articles' + '?id=' + this.props.navigation.state.params.userId,
      last_date: undefined,
      articleList: [],
      more: true,
  ...
  componentDidMount() {
    this.props['screenProps'].navigationEvents.addListener(`onFocus:OriginalArticle`, this.refresh.bind(this))
  }
  ...
  refresh(){
    ...
    fetch(website, {
      ...
      .then( data => {
        console.log('loading originalARticle', data)
        if(data.ret === 0){
          this.setState({
